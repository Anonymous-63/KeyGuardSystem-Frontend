...
export default function ProfilePage() {
  ...
  const { data: locations } = useListLocationsForOperatorQuery(op?.id ?? '', { skip: !op });
  ...
  const [ccCode, setCcCode] = useState('91');
  ...
  useEffect(() => {
    ...
    setCcCode(op.mobileCountryCode ?? '91');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await updateMe({
        body: { name: name.trim(), type: op.type, emailId: email.trim() || undefined, mobileNo: mobile.trim() || undefined, mobileCountryCode: ccCode },
  ...
  const clearance      = clearanceFromType(op.type);
  const clearanceColor = CLEARANCE_COLOR[clearance] ?? '#6b7280';
  const roleName       = OPERATOR_TYPES[op.type] ?? `Type ${op.type}`;
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', marginBottom: '0.75rem' }}>
            ...
            {op.mobileNo && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.55rem', fontSize: '0.78rem' }}>
                ...
                <span style={{ opacity: 0.65 }}>+{op.mobileCountryCode ?? '91'} {op.mobileNo}</span>
              </div>
            )}
          </div>
        </>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '0.35rem' }}>
              <select
                ...
                value={ccCode}
                onChange={(e) => { setCcCode(e.target.value); setDirty(true); }}
                style={{ width: '5.25rem', flexShrink: 0, fontSize: '0.82rem' }}
              >
   ...
   *
   * 2-column grid — 280px left | 1fr right:
   *   Left  (280px): Virink-style profile card, all fits in viewport height.
