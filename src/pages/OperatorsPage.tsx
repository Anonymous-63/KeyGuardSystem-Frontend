...
export default function OperatorsPage() {
  ...
  return (
    <div>
          ...
          <table className="table table-zebra">
            ...
            <tbody>
              ...
              {!isLoading && data?.content.length === 0 && (
                <tr><td colSpan={6} className="text-center py-8 text-base-content/50">No operators found</td></tr>
              )}
              {data?.content.map((op) => (
                <tr key={op.id}>
                  ...
                  <td><span className="badge badge-outline badge-sm">{OPERATOR_TYPES[op.type] ?? `Type ${op.type}`}</span></td>
      ...
      <Modal open={modalOpen} title={editing ? 'Edit Operator' : 'New Operator'} onClose={() => setModalOpen(false)}>
